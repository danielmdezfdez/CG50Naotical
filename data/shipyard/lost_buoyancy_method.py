...
# Iniciamos las variables que no pertenecen a las clases
TotalLostBuoyancy=0.0
TotalDamagedArea=0.0
...
class Ship():
        ...
        def ship_intact_area():
            self.shiparea=self.LOA*self.Beam
            return(self.shiparea)

        def parallel_immersion():
            self.immersion=
...
class Tank():
    ...
    def tank_area():
        self.TankArea=self.EsloraTanque*self.MangaTanque
        return(self.TankArea)
    ...
    def tankvolume(self)
        self.tankvolume=float(self.EsloraTanque*self.MangaTanque*(self.CaladoInicialGdelTanque-self.DFHeight)
        return(self.tankvolume)
...
while DamagedTanksCounter>0:
    ...
    # Funciones que si debemos imprimir
    ...
    # Funciones que no debemos imprimir
    (DamagedTankName.tank_area())    
    TotalDamagedArea=TotalDamagedArea+tank_area()
    TotalLostBuoyancy=TotalLostBuoyancy+(DamagedTankName.lostbuoyancy())

    DamagedTanksCounter=DamagedTanksCounter-1


print("Lost Buoyancy")
print(TotalLostBuoyancy)
